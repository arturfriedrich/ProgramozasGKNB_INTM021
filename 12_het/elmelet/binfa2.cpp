...
void kiirBF(csucs* cs) { // forditott abc sorrend
  if(cs == NULL) return;
  kiirBF(cs->jobb);
  for(long i=0; i<cs->db; i++)
    cout << cs->szoveg << endl;
  kiirBF(cs->bal);
}
...
int main() {
  ...
  cout << "Szavak beolvasasa, tarolasa, "
       << "forditott abc-sorrendben kiirasa\n"
       << "Adja meg a szavakat - vegjelig!\n";
  while(cin>>szo, szo!="-") {
    gyoker = beszurBF(gyoker, szo);
  }
  cout << "A megadott szavak forditott abc "
       << "sorrendben a kovetkezok:\n";
  kiirBF(gyoker);
